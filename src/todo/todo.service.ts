...
export class TodoService {
    ...
    async createTodo(todo: createTodoDto): Promise<todoEntity>{
        try{
            ...
            throw new HttpException('Create new todo error', HttpStatus.INTERNAL_SERVER_ERROR)
        }
    }
    ...
    async deleteTodo(id: number): Promise<number>{
         ...
         */
        if(!foundTodo){
            throw new HttpException('Delete error: Todo not found', HttpStatus.NOT_FOUND);
        }
